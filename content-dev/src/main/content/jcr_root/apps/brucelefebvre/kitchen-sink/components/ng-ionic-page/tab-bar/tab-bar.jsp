...
%>  <ion-tab title="<%= xssAPI.encodeForHTMLAttr(tabBarPage.getTitle()) %>" 
      ...
      ng-click="goTab('<%= request.getContextPath() %><%= xssAPI.getValidHref(tabBarPagePath) %>', '<%= xssAPI.encodeForJSString(tabBarPage.getTitle()) %>', <%= xssAPI.encodeForJSString(Integer.toString(i)) %>)" >
    </ion-tab>
<%
    }
%>
